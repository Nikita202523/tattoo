...
async def handle_jackpot(callback: CallbackQuery):
    ...
    cost = 0 if is_admin else (10 if mode == "jackpot_normal" else 50)
    ...
    # Шансы (меняем при большом балансе)
    if get_balance(user_id) > 199 and not is_admin:
        weights = [50, 25, 15, 8, 2, 0]  # урезаем шансы
    else:
...
def reset_balances_if_needed():
    ...
    # Мягкое обнуление: оставляем 10%
    for uid in balances:
        balances[uid] = round(balances[uid] * 0.1)
